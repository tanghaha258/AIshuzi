...
- [x] P4.1 试讲输入体验：手动发言框支持 `Enter` 发送、`Shift+Enter` 换行，并规避输入法组词期间误发送。
- [x] P5 教师视觉观察第一版：已实现 MediaPipe 按需加载、本地指标推导、观察事件入库、实时建议与观察面板；摄像头拒绝授权降级路径已验收，现场摄像头指标效果待授权试用确认。
- [ ] P6/P7 未开始：报告 2.0、Electron 产品化仍在后续阶段。
